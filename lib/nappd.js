...
Daemon.prototype = {
    ...
    setup: function(args){
        this.daemon = daemonize.setup({
            main: this.path,
            name: this.name,
            stdout: output ? fs.openSync(output, 'w+') : void 0,
    ...
    start: function(args){
        var d = deferred();
        this.setup(args);
        this.daemon.start(function(err, pid){
            if(err) d.reject(err);
            else d.resolve(pid);
        });
    },
    ...
    stop: function () {
        var d = deferred();
        if(!this.daemon){
            d.reject(new Error('Daemon not started yet.'));
            return;
        }
        this.daemon.stop(function(err, pid){
            if(err) d.reject(err);
            else d.resolve(pid);
        });
    },
    ...
    kill: function(){
        ...
        this.daemon.kill(function(err, pid){
            if(err) d.reject(err);
            else d.resolve(pid);
        });
    },
