...
function Config(cfgPath){
    if(!(this instanceof Config)) return new Config(cfgPath);

    if(cfgPath){
        this.cfgPath = path.resolve(cfgPath);
        this.cfgDir = path.dirname(cfgPath);
    }else {
        this.cfgDir = path.join(__dirname, '../cfg');
        this.cfgPath = path.join(this.cfgDir, 'apps.json');
    }
    this.updated = true;
}
Config.prototype = {
    ...
    update: function(cfg){
        ...
        require('mkdirp')(this.cfgDir, function(err){
            if(err) d.reject(err);
            else{
     ...
     *
     * @param app The name of the app to fromRegisteredApp
     */
    getApp: function(app){
        var d = deferred();
