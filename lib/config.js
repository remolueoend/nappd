...
var deferred = require('deferred'),
    path = require('path'),
    fs = require('fs'),
    cfgPath = path.join(__dirname, '../cfg/app.json'),
    updated = true;
    ...
    return function(){
        ...
        if(cfg && !updated) {
            ...
            fs.exists(cfgPath, function(exists){
                if(exists){
                    fs.readFile(cfgPath, 'unicode', function(err, data){
                        if(!err){
                            cfg = JSON.parse(data);
                            updated = false;
                            d.resolve(cfg);
...
function update(cfg){
    var d = deferred();
    fs.writeFile(cfgPath, JSON.stringify(cfg), {encoding: 'unicode'}, function(err){
        if(!err){
            updated = true;
            d.resolve();
        }else{
...
function removeApp(app){
    ...
    getApp(app).then(function(a){
        ...
        d.reject(new Error('No app registered with the name ' + app));
    });
